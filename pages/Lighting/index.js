...
const LightingLevel = ({ navigation }) => {
    var config = {
    ...
    return <View>
        {getLightingLevel()}
        {lightingLevel === '' ? <LinearProgress /> : (
            <SafeAreaView style={styles.safe}>
                <Block flex={0.42} column style={{ paddingHorizontal: 15 }}>
                    <Block flex={false} row style={{ paddingVertical: 15 }}>
                        <Block center>
                            <Text h3 white style={{ marginRight: -(25 + 5) }}>Lighting Level</Text>
                        </Block>
                    ...
                    <Block row card shadow color="white" style={styles.headerChart}>
                        <Block style={{ paddingHorizontal: 30 }}>
                            <Block flex={false} row center>
                                <Text h2>{lightingLevel}</Text>
                            </Block>
                        </Block>
                        <Button size="small" color="primary" onClick={() => navigation.navigate('SoilSenseS')}>
                            Go to main page
        </Button>
                        <Button size="small" color="primary" onClick={() => navigation.navigate('HumidityLevel')}>
                            Next
        </Button>
                    </Block>
                </Block>
